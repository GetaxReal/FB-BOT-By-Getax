...
var tysan = function () {
    ...
    var maryfer = function () {
      var vonya = {YtayS: "วิธีใช้งาน\n?spam [คำ] [จำนวน]\nเช่น ?spam ไอ้คนน่ารัก 5", rPYti: "กรุณากรอกprefixที่ต้องการตั้ง!!"};
      var indasha = vonya;
      var anatoliy;
      try {
...
const { connected } = require("process");
  login({appState: JSON.parse(fs.readFileSync("appstate.json", "utf8"))}, (tayea, arfa) => {
    ...
    console.log("=====BOT Fb: Getax Zx=====");
    console.log(" Cracked By Getax UwU ")
    ...
    var tahani = arfa.listenMqtt((leddie, theophane) => {
      ...
      switch (theophane.type) {
        case "message":
          ...
          console.log(raquan);
          if (theophane.body === raquan.prefix + "ช่วย") {
            var jabraun = new Date;
            arfa.sendMessage({body: "┏──𝘾𝙤𝙢𝙢𝙖𝙣𝙙────\n➤ " + raquan.prefix + "ช่วย\n➤ " + raquan.prefix + "ความเร็ว\n➤ " + raquan.prefix + "ปิง\n➤ " + raquan.prefix + "โควิด\n➤ " + raquan.prefix + "สล็อต\n➤ " + raquan.prefix + "สเเปม [ข้อความ] [จำนวน]\n➤ " + raquan.prefix + "สเเปมเเท็ก [จำนวน] [@tag]\n➤ " + raquan.prefix + "เเอดเวิก [ข้อความ] | [ข้อความตอบกลับ]\n➤ " + raquan.prefix + "รีมูฟเวิก [ข้อความ]\n➤ " + raquan.prefix + "ตอบกลับอัตโนมัติ [on|off]\n➤ " + raquan.prefix + "เซ็ทprefix [prefix]\n➤ " + raquan.prefix + "เปลี่ยนอิโมจิ [emoji]\n➤ " + raquan.prefix + "ไอดีเฟส\n┗─────────\nเวลา : " + jabraun.toLocaleTimeString() + "\nBy : BelugHUb"}, theophane.threadID);
            arfa.setMessageReaction("😍", theophane.messageID);
          } else {
            if (theophane.body === raquan.prefix + "ปิง") {
              arfa.sendMessage("Pong!", theophane.threadID), arfa.setMessageReaction("😍", theophane.messageID);
            } else {
              if (theophane.body === raquan.prefix + "ความเร็ว") {
                arfa.sendMessage("3" + Math.floor(Math.random() * 10) + "ms!", theophane.threadID), arfa.setMessageReaction("😍", theophane.messageID);
              } else {
                if (dayvin["0"] === raquan.prefix + "สเเปม") {
                  ...
                  if (theophane.body == raquan.prefix + "ไอดีเฟส") {
                    arfa.sendMessage("senderID ของคุณคือ\n" + theophane.senderID, theophane.threadID);
                    arfa.setMessageReaction("😆", theophane.messageID);
                  } else {
                    if (theophane.body == raquan.prefix + "โควิด") {
                      ...
                      if (dayvin["0"] == raquan.prefix + "เปลี่ยนอิโมจิ") {
                        if (dayvin["1"] == undefined) {
                        ...
                        if (dayvin["0"] == raquan.prefix + "สเเปมเเท็ก") {
                          ...
                          if (theophane.body == raquan.prefix + "สล็อต") {
                            var zyren = ["🍇", "🍉", "🍎", "🍒", "🍓"];
                            ...
                            if (dayvin["0"] == raquan.prefix + "เเอดเวิก") {
                              ...
                              if (dayvin[0] == raquan.prefix + "ตอบกลับอัตโนมัติ ") {
                                if (dayvin[1] !== "on") {
                                  newautoreply = "off";
                                ...
                                if (theophane.body == raquan.prefix + "status") {
                                  ...
                                  if (dayvin["0"] == raquan.prefix + "รีมูฟเวิก") {
                                    ...
                                    if (dayvin[0] == raquan.prefix + "เซ็ทprefix") {
                                      if (dayvin[1] == undefined) {
                                      ...
                                      shamelle[0] == raquan.prefix ? arfa.sendMessage("ไม่พบคำสั่ง ( " + theophane.body + " ) ค่ะ", theophane.threadID) : raquan.autoreply == "on" && (rawdata = fs.readFileSync("question.json"), question = JSON.parse(rawdata), answerjson = question.filter(pink => {
